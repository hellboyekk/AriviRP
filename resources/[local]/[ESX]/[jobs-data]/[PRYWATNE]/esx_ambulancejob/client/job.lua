    ...
    while true do
        ...
        for i = 1, #Config.Doctor, 1 do
            ...
            if userDst <= 7 then
                sleep = 2
                if userDst <= 5 then
					ESX.ShowHelpNotification('Naciśnij ~INPUT_CONTEXT~ aby skorzystać z pomocy lekarza za: $'..Config.doctorPrice)
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
				...
				if data.current.value == 'extras' then
					print('działa')
					exports['esx_policejob']:OpenExtraMenu()
				end
...
AddEventHandler('esx_ambulancejob:hasEnteredMarker', function(zone)
  ...
  elseif zone == 'HeliSpawner' then
	...
	CurrentActionData = {array = Config.Zones.HeliSpawnPoint, plate = "EMS " .. GetRandomIntInRange(1000,9999), livery = 0}
  ...
  elseif zone == 'HeliSpawner2' then
	CurrentAction	 = 'heli_spawner'
	CurrentActionMsg  = 'Naciśnij ~INPUT_CONTEXT~ aby wezwać helikopter'
	CurrentActionData = {array = Config.Zones.HeliSpawnPoint2, plate = "EMS " .. GetRandomIntInRange(1000,9999), livery = 0}
  elseif zone == 'HeliSpawner3' then
	CurrentAction	 = 'heli_spawner'
	CurrentActionMsg  = 'Naciśnij ~INPUT_CONTEXT~ aby wezwać helikopter'
	CurrentActionData = {array = Config.Zones.HeliSpawnPoint3, plate = "EMS " .. GetRandomIntInRange(1000,9999), livery = 0}
  elseif zone == 'HeliSpawner4' then
	CurrentAction	 = 'heli_spawner'
	CurrentActionMsg  = 'Naciśnij ~INPUT_CONTEXT~ aby wezwać helikopter'
	CurrentActionData = {array = Config.Zones.HeliSpawnPoint4, plate = "EMS " .. GetRandomIntInRange(1000,9999), livery = 0}
  elseif zone == 'HeliSpawner5' then
	CurrentAction	 = 'heli_spawner'
	CurrentActionMsg  = 'Naciśnij ~INPUT_CONTEXT~ aby wezwać helikopter'
	CurrentActionData = {array = Config.Zones.HeliSpawnPoint5, plate = "EMS " .. GetRandomIntInRange(1000,9999), livery = 0}
