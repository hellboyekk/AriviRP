      ...
      if data.current.value == 'vehicle_list' then
        ...
        if Config.EnableSocietyOwnedVehicles then
            ...
            local elements = {
              ...
              {label = 'Laweta', value = 'bc205500w'},
              {label = 'Laweta', value = 'bclandscape'},
              {label = 'Pojazd Służbowy', value = 'cateyelifted'},
              {label = 'Pojazd Służbowy', value = 'f350worktruck'}
            }
...
function OpenMechanikShop()
    ...
    ESX.UI.Menu.Open(
        ...
        {
			align    = 'center',
            title    = 'Bennys - Żywność',
            elements = {
				{label = 'Shake Proteinowy', value = 'protein_shake'},
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
          if data.current.value == 'gazbottle' then
            TriggerServerEvent('esx_mecanojob:giveItem', data.current.value)
...
function PrzebierzSzatnia(styl)
  ...
  if styl == 'ubranieK1' then
    SetPedComponentVariation(GetPlayerPed(-1), 8, 56, 0, 2)   -- PODKOSZULEK
    SetPedComponentVariation(GetPlayerPed(-1), 11, 136, 3, 2)  -- TOP
    SetPedComponentVariation(GetPlayerPed(-1), 3, 62, 0, 2)   -- RĘCE
    SetPedComponentVariation(GetPlayerPed(-1), 4, 61, 0, 2)   -- SPODNIE
    SetPedComponentVariation(GetPlayerPed(-1), 6, 24, 1, 2)   -- BUTY
  elseif styl == 'ubranieM1' then
    SetPedComponentVariation(GetPlayerPed(-1), 8, 140, 1, 2)   -- PODKOSZULEK
    SetPedComponentVariation(GetPlayerPed(-1), 11, 247, 0, 2)  -- TOP
    SetPedComponentVariation(GetPlayerPed(-1), 3, 52, 0, 2)   -- RĘCE
    SetPedComponentVariation(GetPlayerPed(-1), 4, 97, 1, 2)   -- SPODNIE
    SetPedComponentVariation(GetPlayerPed(-1), 6, 24, 0, 2)   -- BUTY
...
Citizen.CreateThread(function()
  while true do
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'mecano' then
      ...
      local coords, sleep = GetEntityCoords(GetPlayerPed(-1)), true
