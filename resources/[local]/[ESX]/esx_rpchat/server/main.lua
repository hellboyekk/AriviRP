...
RegisterCommand('try', function(source, args, rawCommand)
	...
	exports.AriviRP:discord('\n**/TRY LOGI**\n \n **'.. GetPlayerName(source) ..'** '.. table.concat(args, " ") ..'\n** Serwer ID:**'.. source ..'**\nSteam ID:** '.. GetPlayerIdentifiers(source)[1] ..'', '3738742', 'try')
end)

RegisterServerEvent('szymczakovv:checkblck')
AddEventHandler('szymczakovv:checkblck', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local checkinghere = 1
	local result = MySQL.Sync.fetchAll(
		'SELECT blockmessages FROM user_lastcharacter WHERE identifier = @identifier', {
			['@identifier'] = xPlayer.identifier,
			['@blackmessages'] = checkinghere
		})
	if result[1] == checkinghere then
		xPlayer.showNotification('~o~Nie możesz wysłać wiadomości - Twoje konto dostało "MUTE na WIADOMOŚCI"')
	end
end)
...
RegisterCommand('me', function(source, args, rawCommand)
	...
	exports.AriviRP:discord('\n**/ME LOGI**\n \n **'.. GetPlayerName(source) ..'** '.. text ..'\n** Serwer ID:**'.. source ..'**\nSteam ID:** '.. GetPlayerIdentifiers(source)[2] ..'', '3738742', 'me')
...
RegisterCommand('do', function(source, args, rawCommand)
	...
	exports.AriviRP:discord('\n**/DO LOGI**\n \n **'.. GetPlayerName(source) ..'** '.. text ..'\n** Serwer ID:**'.. source ..'**\nSteam ID:** '.. GetPlayerIdentifiers(source)[2] ..'', '3757298', 'odo')
	TriggerClientEvent('esx_rpchat:triggerDisplay', -1, text, source, color)
end, false)
...
RegisterServerEvent('sendTwtServer')
AddEventHandler('sendTwtServer', function(message)
	local _source = source
	...
	if result[1] == checkinghere then
		xPlayer.showNotification('~o~Nie możesz wysłać wiadomości - Twoje konto dostało BLOKADĘ od Administratora!')
	else
	...
	exports.AriviRP:discord('\n**/TWT LOGI**\n \n **'.. GetPlayerName(_source) ..'** '.. message ..'\n** Serwer ID:**'.. _source ..'**\nSteam ID:** '.. GetPlayerIdentifiers(_source)[2] ..'', '7129087', 'twt')
end)
...
RegisterCommand('o', function(source, args, rawCommand)
	if args[1] ~= nil then
		local xPlayer = ESX.GetPlayerFromId(source)
		if xPlayer.group == 'superadmin' or xPlayer.group == 'admin' or xPlayer.group == 'best' or xPlayer.group == 'moderator' then
			local playerName = GetPlayerName(source)
			msg = Emojit(table.concat(args, " "))
			TriggerClientEvent('chatMessage', -1, "🌐 ^1".. playerName, {30, 144, 255}, msg)
			exports.AriviRP:discord('\n**OGŁOSZENIA SERWERA LOGI**\n \n **'.. GetPlayerName(source) ..'** '.. msg ..'\n** Serwer ID:**'.. source ..'**\nSteam ID:** '.. GetPlayerIdentifiers(source)[1] ..'', '16711680', 'serwer')
		end
	end
end)

-- Ogłoszenia dla wszystkich
RegisterCommand('news', function(source, args, rawCommand)
	...
	if quantity > 0 then
		...
		exports.AriviRP:discord('\n**/NEWS LOGI**\n \n **'.. GetPlayerName(source) ..'** '.. msg ..'\n** Serwer ID:**'.. source ..'**\nSteam ID:** '.. GetPlayerIdentifiers(source)[1] ..'', '7126272', 'news')
	end
end, false)

RegisterServerEvent('senddarkServer')
AddEventHandler('senddarkServer', function(message)
	local _source = source
    if CheckPhone(_source) <= 0 then
        TriggerClientEvent('esx:showNotification', _source, '~r~Nie posiadasz telefonu.')
        return
	...
	exports.AriviRP:discord('\n**/DW LOGI**\n \n **'.. GetPlayerName(_source) ..'** '.. message ..'\n** Serwer ID:**'.. _source ..'**\nSteam ID:** '.. GetPlayerIdentifiers(_source)[2] ..'', '7129087', 'dw')
end)

function opissunio(hook,message,color)
    local e123asyadmin21 = 'https://discord.com/api/webhooks/815006297422430268/XIcRBHugHq7gOun56LOTGkF-zrwukzoxqT0U-28W-YaA5n8rd1Cs3TJvVQTweRTrQIrq'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
...
RegisterCommand('opis', function(source, args, user)
	...
	if args[1] ~= nil then
		...
		TriggerClientEvent('esx:showNotification', source, '~r~Maksymalna długość opisu postaci wynosi ~h~60~h~ znaków!~w~')
			...
			TriggerClientEvent('elfeedo_opis:Description', -1, source, ' '..text..' ')
			...
			opissunio('AriviRP.pl', wiadomosc, 11750815)
		end
	else
		TriggerClientEvent('elfeedo_opis:Description', -1, source, '')
...
TriggerClientEvent('elfeedo_opis:Description', -1, source, iSource)
TriggerClientEvent('elfeedo_opis:PlayersDescription', -1)
end)

RegisterServerEvent('elfeedo_opis:PlayersDescriptionServer')
AddEventHandler('elfeedo_opis:PlayersDescriptionServer', function(id, i)
TriggerClientEvent('elfeedo_opis:Description', -1, id, i)
end)

ESX.RegisterUsableItem('pierscionek1', function(source)
TriggerClientEvent('elfeedo_opis:Description', -1, source, "Na palcu u lewej dłoni widać pierścionek zaręczynowy")
...
TriggerClientEvent('elfeedo_opis:Description', -1, source, "Na palcu u prawej dłoni widać pierścionek ślubny")
