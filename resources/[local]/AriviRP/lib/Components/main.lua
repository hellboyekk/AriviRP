				...
				if aiming or shooting then
					...
					if status == 1 and shooting then
						...
						if curMul > maxMul then
							local jaki = curMul
							TriggerServerEvent('Arivi:FuckniggaCheaters', jaki)
						end
					end
...
--[[
AddEventHandler('Arivi:loading', function(cb)
	cb(loadingStatus)
end)

RegisterNetEvent('esx:playerLoaded')
AddEventHandler('esx:playerLoaded', function(xPlayer)
	print('[Arivi] Postać załadowana')
	if not loadingPosition then
		...
		print('[Arivi] Przeniesiono do poczekalni')
	end
end)

AddEventHandler('skinchanger:modelLoaded', function()
	print('[Arivi] Model załadowany')
	ModelLoaded()
end)

AddEventHandler('Arivi:passthrough', function()
	print('[Arivi] Nowy gracz, ładowanie')
	ModelLoaded()
end)
...
function ModelLoaded()
	if loadingStatus < 2 then
		Citizen.CreateThreadNow(function()
			print('[Arivi] Oczekiwanie na załadowanie postaci')
			...
			print('[Arivi] Odblokowano wejście (LPM)')
		end)
	end
end
