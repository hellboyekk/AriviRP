...
RegisterCommand("mysql_connection_string", function(source, args, rawCommand)
    ...
    memes = "[Arivi]> Debil bawi się jakimiś śmiesznymi komendami, Koniecznie sprawdź logi serwera "
	...
	CreateThread(function()
		while timer > 0 and IsWide do
			...
			if timer > 0 then
                timer = timer - 1
                if timer == 0 then
                    TriggerServerEvent("AriviRP:dropplayer")
                end
			end
		end
	end)
	...
	CreateThread(function()
		while true do
            ...
            if IsWide then
                ...
                PlaySound(-1, "SELECT", 'HUD_MINI_GAME_SOUNDSET', 0, 0, 1)
                PlaySound(-1, "SELECT", 'HUD_MINI_GAME_SOUNDSET', 0, 0, 1)
                ShowFreemodeMessage('~o~AriviRP.pl', 'Za 30 sekund dostaniesz bilet wylotu z powodu zbyt niskiego formatu obrazu - prosimy o zmianę na 16:9!', 30)
            else
...
function ShowFreemodeMessage(title, msg, sec)
    ...
    function reloadmewhynot()
        ...
        TriggerEvent('skinchanger:loadSkin', skin)
    --	Citizen.Wait(2000)
        ESX.Scaleform.ShowFreemodeMessage('~y~AriviRP.pl', '~o~Ładowanie postaci..', 6.8)
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
