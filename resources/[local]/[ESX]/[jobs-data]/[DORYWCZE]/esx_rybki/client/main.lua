	...
	while true do
		...
		if PlayerData.job.name == 'rybak' then
			...
			while IsFishing do
				...
				local time = math.random(4*3000, 4*4500)
				TaskStandStill(GetPed(), time+math.random(7000,10000))
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
			if data.current.value == 'pal' then
				local coords = GetEntityCoords(GetPlayerPed(-1))
				if not exports['esx_policejob']:isHandcuffed() and not exports['esx_ambulancejob']:getDeathStatus() then
