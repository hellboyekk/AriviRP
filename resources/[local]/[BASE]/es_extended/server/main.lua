...
AddEventHandler('esx:gitestveInventoryItem', function(target, type, itemName, itemCount)
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetItem.limit ~= -1 and (targetItem.count + itemCount) > targetItem.limit then
				...
				TriggerClientEvent('sendProximityMessageDo', -1, playerId, playerId, "przekazał "..sourceItem.label.." w ilości ["..itemCount.."]")
					...
					giveitem('AriviRP.pl', wiadomosc, 11750815)
			end
		else
	...
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			givepieniadze('AriviRP.pl', wiadomosc, 11750815)
		else
		...
		if sourceXPlayer.hasWeapon(itemName) then
			...
			if not targetXPlayer.hasWeapon(itemName) then
				...
				if weaponObject.ammo and itemCount > 0 then
					...
					local steamid = xPlayer.identifier
					local na2me = GetPlayerName(playerId)
					wiadomosc = "Przekazano pieniądze: "..weaponLabel.." x"..itemCount.." DLA: "..target.." "..GetPlayerName(target).."\n[ID: "..source.." | Nazwa Steam: "..na2me.." | ROCKSTAR: "..steamid.." ]" 
					giverweapon('AriviRP.pl', wiadomosc, 11750815)
				else
					...
					local steamid = xPlayer.identifier
					local na2me = GetPlayerName(playerId)
					wiadomosc = "Przekazano pieniądze: "..weaponLabel.." DLA: "..target.." "..GetPlayerName(target).."\n[ID: "..source.." | Nazwa Steam: "..na2me.." | ROCKSTAR: "..steamid.." ]" 
					giverweapon('AriviRP.pl', wiadomosc, 11750815)
				end
			else
...
function giveitem(hook,message,color)
    local jebaczydufiniggerow = 'https://discord.com/api/webhooks/812274179767795742/oGHiOpkZOAx4u4VxQSkJum6FJrcnuiZFpW_4qp1Jdw0hNxv2boiffEl0VcRff9R_a_xF'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
...
function givepieniadze(hook,message,color)
    local krzychufaza = 'https://discord.com/api/webhooks/812275437169278996/LDEmYDfYpG6Q-rE3i3tQMBEgIgI9cmb5a_5cpPFP_qmxBbaZetn0nPXxHLGSxns7pQbl'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
...
function giverweapon(hook,message,color)
    local krzychufazapedal = 'https://discord.com/api/webhooks/812275627906170971/yEmraILRmBMk0mfI0gMj0xmmOJEu0lT7es3t_el2-RUpmRKKMYiQoL0sTTHdEPVApL6N'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
