        ...
        while CruisedSpeed > 0 and GetPedInVehicleSeat(vehicle, -1) == ped do
          ...
          if not IsControlPressed(2, 76) and not IsControlPressed(2, 63) and not IsControlPressed(2, 64) then
            if speed < (CruisedSpeed - 5.0) then
              CruisedSpeed = 0
              ESX.ShowNotification('Wyłączono tempomat')
              Citizen.Wait(2000)
              break
            end
...
local timescreenPosX =  0.135
local timescreenPosY =  0.72

local timecolor = {253, 252, 155} 
...
CreateThread(function()
    ...
    while true do
        ...
        if pedInVeh or exports['gcphone']:getMenuIsOpen() or exports['EasyAdmin']:NoClip() then
            drawTxt(timeText, 4, timecolor, 0.4, timescreenPosX, timescreenPosY + 0.048)
    ...
    while true do
      Citizen.Wait(1)
        if pedInVeh or exports['gcphone']:getMenuIsOpen() or exports['EasyAdmin']:NoClip() then
            ...
            timeText = ("%.2d"):format((hour == 0) and 23 or hour) .. ":" .. ("%.2d"):format( minute) .. ((hour < 00) and " " or " ")
        end
    end
end)
