...
--[[function OpenPsmActionsMenu()  -- MENU SZEFA OFF DO CZASU FIRM
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'psm_actions', {
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'piekarz', function(data, menu)
					menu.close()
				end)
			end
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_piekarz:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_piekarz:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_piekarz:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_piekarz:putStockItems', itemName, count)
						end
					end, function(data2, menu2)
						menu2.close()
					end)
...
AddEventHandler('esx_piekarz:hasEnteredMarker', function(zone, station, partNum)
	if zone == 'Material' and PlayerData.job ~= nil and PlayerData.job.name == 'piekarz' and onDuty == true then
	...
	if zone == 'PsmActions' and PlayerData.job ~= nil and PlayerData.job.name == 'piekarz' then
		CurrentAction     = 'psm_actions_menu'
		CurrentActionMsg  = _U('press_to_open_boss')
		CurrentActionData = {}
	end
  
	if zone == 'Cloakrooms' and PlayerData.job ~= nil and PlayerData.job.name == 'piekarz' then
		CurrentAction     = 'cloakroom_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end

	if zone == 'Vehicles' and PlayerData.job ~= nil and PlayerData.job.name == 'piekarz' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {station = station, partNum = partNum}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'piekarz' then
...
AddEventHandler('esx_piekarz:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'Material') and PlayerData.job ~= nil and PlayerData.job.name == 'piekarz' then
		TriggerServerEvent('esx_piekarz:stopHarvest')
	end  
	if (zone == 'Szycie' or zone == 'Pakowanie') and PlayerData.job ~= nil and PlayerData.job.name == 'piekarz' then
		TriggerServerEvent('esx_piekarz:stopTransform')
		TriggerServerEvent('esx_piekarz:stopTransform2')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'piekarz' then
		TriggerServerEvent('esx_piekarz:stopSell')
	end
	CurrentAction = nil
	hintIsShowed = false
end)
...
function blips()
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'piekarz' then
	for k,v in pairs(Config.Baza)do
			if v.Type == 1 then
				local blip2 = AddBlipForCoord(v.Pos.x, v.Pos.y, v.Pos.z)
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(Config.Zones) do
			if PlayerData.job ~= nil and PlayerData.job.name == 'piekarz' and onDuty == true then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'piekarz' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentPart) then
				...
				TriggerEvent('esx_piekarz:hasEnteredMarker', currentPart, currentStation, currentPartNum)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_piekarz:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and isInZone then
			TriggerEvent('esx_piekarz:hasEnteredMarker', 'exitMarker')
		end
	end
end)
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				if CurrentAction == 'exitMarker' then
					...
					TriggerEvent('esx_piekarz:freezePlayer', false)
					TriggerEvent('esx_piekarz:hasExitedMarker', lastZone)
					Citizen.Wait(15000)
				...
				elseif CurrentAction == 'szycie' then
					TriggerServerEvent('esx_piekarz:startTransform', CurrentActionData.zone)
				...
				elseif CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_piekarz:startSell', CurrentActionData.zone)
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					TriggerEvent('esx_piekarz:freezePlayer', false)
							if Config.EnableSocietyOwnedVehicles then
								...
								TriggerServerEvent('esx_society:putVehicleInGarage', 'piekarz', vehicleProps)
							end
				...
				if isInZone then
					TriggerEvent('esx_piekarz:freezePlayer', true)
				end
...
RegisterNetEvent('esx_piekarz:freezePlayer')
AddEventHandler('esx_piekarz:freezePlayer', function(freeze)
