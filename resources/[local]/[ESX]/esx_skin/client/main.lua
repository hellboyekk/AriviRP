...
function CreateSkinCam(switch)
  ...
  Citizen.CreateThread(function()
	while true do
	  ...
	  elseif hasCam then
		SetTextComponentFormat("STRING")
		AddTextComponentString(_U('rotate_view'))
		DisplayHelpTextFromStringLabel(0, 0, 0, -1)
	  end
	end
  end)
  ...
  Citizen.CreateThread(function()
	...
	while true do
	  Citizen.Wait(0)
	  if cam then
		if IsDisabledControlJustPressed(1, 0) then
		  DeleteSkinCam(true)
		...
		heading = angle + 0.0
	  elseif hasCam and IsDisabledControlJustPressed(1, 0) then
		CreateSkinCam(true)
	  end
	end
  end)
