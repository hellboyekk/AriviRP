...
AddEventHandler('onResourceStart', function(resourceName)
	...
	local realname = 'szymczakovv_stocks'
...
AddEventHandler('szymczakovv_stocks:getItemInStock', function(type, item, count, name)
	...
	wiadomosc = "WYCIĄGNĄŁ Z SZAFKI \n[ITEM: "..item.." | ILOSC: x"..count.."] \n[ID: "..source.." | Nazwa: "..name2.." | SteamID: "..steamid.."]" 
	DiscordHookwyciaganie3('AriviRP.pl', wiadomosc, 11750815)
end)
...
AddEventHandler('szymczakovv_stocks:getWeaponInStock', function(item, count, name)
	...
	wiadomosc = name2.." WYCIĄGNĄŁ Z SZAFKI \n[Broń: "..item.." | ILOSC: x"..count.."] \n[ID: "..source.." | Nazwa: "..name2.." | SteamID: "..steamid.." ]" 
	DiscordHookwyciaganie('AriviRP.pl', wiadomosc, 11750815)
end)
...
AddEventHandler('szymczakovv_stocks:putItemInStock', function(type, item, count, name)
	...
	if playerItemCount >= count and count > 0 then
		putItemInStock(_source, count, item, name)
		local xPlayer = ESX.GetPlayerFromId(source)
		local steamid2 = xPlayer.identifier
		local name2 = GetPlayerName(source)
		putWeaponInStock(_source, {name=item, ammo=count}, name)
		wiadomosc = name2.." WSADZIŁ DO SZAFKI \n[ITEM: "..item.." | ILOSC: x"..count.."] \n[ID: "..source.." | Nazwa: "..name2.." | SteamID: "..steamid2.." ]" 
		DiscordHookwkladanie2('AriviRP.pl', wiadomosc, 11750815)
	else
...
AddEventHandler('szymczakovv_stocks:putWeaponInStock', function(item, count, name)
	...
	putWeaponInStock(_source, {name=item, ammo=count}, name)
	wiadomosc = name2.." WSADZIŁ DO SZAFKI \n[Broń: "..item.." | ILOSC: x"..count.."] \n[ID: "..source.." | Nazwa: "..name2.." | SteamID: "..steamid.." ]" 
	DiscordHookwkladanie('AriviRP.pl', wiadomosc, 11750815)
end)
...
function DiscordHookwyciaganie(hook,message,color)
    local wyciaganie = 'https://discord.com/api/webhooks/802727281772462150/ml41iI6e1Mg595Xx-FHLreReDDX3yIKY2W7fBfqx-WWX3gzW_Ecz6YFaltHGruVHKwme'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
    ...
    if message == nil or message == '' then return FALSE end
    PerformHttpRequest(wyciaganie, function(err, text, headers) end, 'POST', json.encode({ username = hook,embeds = embeds}), { ['Content-Type'] = 'application/json' })
end

function DiscordHookwyciaganie3(hook,message,color)
    local wyciaganie44 = 'https://discord.com/api/webhooks/802916214874701825/4GhJ8GneKLzNku0gv9njzIJ14dKdE7o2geGJ0TfZMBBKOlw7BPoqUKQexgbnLs3kOtFH'
    local embeds = {
                {
            ["title"] = message,
            ["type"] = "rich",
