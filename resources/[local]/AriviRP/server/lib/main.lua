...
RegisterServerEvent('Arivi:GetJobsDuty')
AddEventHandler('Arivi:GetJobsDuty', function()
...
RegisterServerEvent('Arivi:GetJobsLicense')
AddEventHandler('Arivi:GetJobsLicense', function()
...
RegisterServerEvent('AriviGetJobsInsuraceEMS')
AddEventHandler('AriviGetJobsInsuraceEMS', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local result = MySQL.Sync.fetchAll(
        'SELECT type, owner FROM user_licenses WHERE owner = @owner AND type = @type',
        {
...
RegisterServerEvent('AriviGetJobsInsuraceLSC')
AddEventHandler('AriviGetJobsInsuraceLSC', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local result = MySQL.Sync.fetchAll(
        'SELECT type, owner FROM user_licenses WHERE owner = @owner AND type = @type',
        {
